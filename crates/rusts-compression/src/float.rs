    ...
    pub fn encode(&mut self, value: f64) {
        ...
        if xor == 0 {
            ...
            if leading >= self.prev_leading && trailing >= self.prev_trailing {
                ...
                self.writer.write_bits(meaningful, meaningful_bits as usize);
            } else {
...
impl<'a> GorillaDecoder<'a> {
    ...
    pub fn decode(&mut self) -> Result<Option<f64>> {
        ...
        let bits = if self.count == 0 {
            ...
            let same = !self.reader.read_bit()?;

            if same {
                // Same value
                self.prev_value
            } else {
                let new_block = self.reader.read_bit()?;
                ...
                let meaningful = self.reader.read_bits(meaningful_bits as usize)?;
